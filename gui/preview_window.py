...
from core.parser import parse_nights
...
class PreviewWindow(ctk.CTkToplevel):
    def _create_frame(self, parent=None):
        return ctk.CTkFrame(parent or self, fg_color="transparent")
    ...
    def __init__(self, parent, result):
        ...
        self.result = result
        self.parsed = result.get("parsed", {})
        self.nights = self.parsed["nights"]

        self.config_mgr = ConfigManager()
        self.units_config = self.config_mgr.get_all_units()

        # Fallback if parsed["units"] is empty or None
        parsed_units = self.parsed.get("units") or []
        if parsed_units:
            self.last_room_used = parsed_units[0]
        elif self.units_config:
            self.last_room_used = list(self.units_config.keys())[0]
        else:
            self.last_room_used = ""
    ...
    def _build_people(self):
        people = []
        default_room = self.last_room_used

        # Fallback to 1 adult if parsed adults is None/0
        adults_count = self.parsed.get("adults") or 1
        for _ in range(adults_count):
            people.append({"type": "adult", "room": default_room})

        for age in self.parsed.get("children_ages") or []:
            people.append({
                "type": "child",
                "age": age,
                "room": default_room
    ...
    def _build_layout(self):
        self.container = self._create_frame(self)
    ...
    def _build_booking_section(self):
        ...
        first_name = self.parsed.get("first_name") or ""
        surname_val = self.parsed.get("surname") or ""
        full = f"{first_name} {surname_val}".strip()
        ...
        self.surname.insert(0, surname_val)
        ...
        self.checkin.insert(0, self.parsed.get("check_in") or "")
        ...
        self.checkout.insert(0, self.parsed.get("check_out") or "")
    ...
    def _build_guest_section(self):
        guest_block = self._create_frame(self.container)
        ...
        controls = self._create_frame(guest_block)
    ...
    def _build_price_section(self):
        price_block = self._create_frame(self.container)
    ...
    def _build_template_section(self):
        frame = self._create_frame(self.container)
    ...
    def _recalculate(self):
        ...
        for p in self.people:
            selected_room = p["widget"].get()
            if selected_room in self.room_name_to_id:
                p["room"] = self.room_name_to_id[selected_room]
    ...
    def _copy_email(self):
        if "email" in self.result and "body_html" in self.result["email"]:
            copy_email_to_clipboard(self.result["email"]["body_html"])

    def _copy_subject(self):
        if "email" in self.result and "subject" in self.result["email"]:
            pyperclip.copy(self.result["email"]["subject"])

    def _copy_email_id(self):
        pyperclip.copy(self.parsed.get("email") or "")